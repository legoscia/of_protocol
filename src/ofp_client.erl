...
%%
%% For more information on `ControllerHandle' @see ofp_channel:open/4.
-spec start_link(Tid :: ets:tid(), ResourceId :: string(),
                 ControllerHandle ::
                   {remote_peer, inet:ip_address(), inter:port_number(), Proto} |
...
%% message. For more information on `ControllerHandle' @see ofp_channel:open/4.
    ...
    case of_protocol:decode(<<Buffer/binary, Data/binary>>) of
        ...
        {error, binary_too_small} ->
            {noreply, State#state{hello_buffer = <<Buffer/binary,
                                                   Data/binary>>}};
        _Else ->
            terminate_connection(State, bad_initial_message)
    end;
handle_info({Type, Socket, Data}, #state{id = Id,
                                         controller = {Host, Port, Proto},
                                         socket = Socket,
  ...
  when Type == tcp orelse Type == ssl ->
    ...
    case of_protocol:decode(Data) of
        {ok, #ofp_message{version = Version} = Message, _Leftovers} ->
            ...
            case Message2#ofp_message.type of
                error_msg ->
                    terminate_connection(State, {failed_negotation, Version});
                _Else ->
                    Parent ! {ofp_connected, self(),
                              {Host, Port, Id, Version}},
        ...
        _Else ->
            terminate_connection(State, bad_message)
    end;
